        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            Varyings Vert(Attributes v)
            {
                ...
                displacedOS.xyz += mul(unity_WorldToObject, float4(offset, 0.0)).xyz;

                float3 worldPos = mul(unity_ObjectToWorld, displacedOS).xyz;
                float3 worldNormal = ComputeGerstnerNormal(tangent, binormal);

                float4 positionCS = TransformWorldToHClip(worldPos);

                o.positionCS = positionCS;
                o.screenUv = GetNormalizedScreenSpaceUV(positionCS);
                o.worldPos = worldPos;
